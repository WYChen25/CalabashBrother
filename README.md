...
- 2018-12-31
- 增加战斗回放时显示回合数
- 修复了回放结束后无法开始游戏的问题，现在可以在回放结束后手动选择阵容开始新游戏。
- 删除了不必要的输出
...
## 设计说明
- 实现类BattleRecord,用于战斗记录与回放
- 实现菜单栏file目录下的recordFile在战斗结束时可用，点击时弹出文件选择框，选择要保存的位置及文件名保存本次战斗过程
- 实现菜单栏file目录下的readFile在战斗结束时可用，点击弹出文件选择框，选择要重放的文件还原战斗过程(用户手动按下A键实现播放)

## 类结构
- 实现battleRecord类，包含saveFile于recordFile,以及用于临时存储战斗信息的list。

## 用户说明
- 现在可在战斗结束后点击菜单栏file目录下的recordFile保存文件，或是点击readFile播放指定文件所记载的战斗记录。
